...
export const metadata = buildMetadata({
  title: "Projects",
  description:
    "Selected Nexatel projects across fiber optics, telecom infrastructure, network cabling, CCTV systems and IT hardware in India.",
  path: "/projects",
  ...
  return (
    <>
      <Hero
        ...
        subtitle="A short selection of recent Nexatel deployments across India, with primary delivery from our Kerala HQ."
        size="compact"
        showStatus={false}
          ...
          {projects.map((p) => (
            <article key={p.id} className="card overflow-hidden flex flex-col">
              <div className="relative aspect-[16/10] bg-[var(--surface)]">
                ...
                <div className="absolute inset-0" aria-hidden style={{ background: "linear-gradient(180deg, transparent 50%, rgba(5,8,22,0.55))" }} />
              </div>
              <div className="p-6 flex flex-col gap-2 flex-1">
                ...
                <p className="text-sm text-slate-400 mt-1.5 leading-relaxed">{p.summary}</p>
              </div>
            </article>
          ))}
        </div>
